...
export class Connection {
  ...
  initializeApi = async (
    ...
    try {
      if (Connection.isExecuting.value)
        return { error: 'Already in execution.', value: true }

      Connection.isExecuting.value = true
      //create the Testnet Connection
      if (
        networkMode.toLowerCase() === NETWORK.TEST_NETWORK.toLowerCase() ||
        bothInit
      ) {
        if (!Connection.nativeApiTestnet)
          Connection.nativeApiTestnet = await this.createNativeConnection(
            networkTest,
          )
        if (!Connection.evmApiTestnet)
          Connection.evmApiTestnet = this.createEvmConnection(networkTest)
      ...
      if (
        networkMode.toLowerCase() === NETWORK.QA_NETWORK.toLowerCase() ||
        bothInit
      ) {
        if (!Connection.nativeApiQA)
          Connection.nativeApiQA = await this.createNativeConnection(networkQA)
        if (!Connection.evmApiQA)
          Connection.evmApiQA = this.createEvmConnection(networkQA)
      ...
      if (networkMode.toLowerCase() === NETWORK.UAT.toLowerCase() || bothInit) {
        if (!Connection.nativeApiUAT)
          Connection.nativeApiUAT = await this.createNativeConnection(
            networkUAT,
          )
        if (!Connection.evmApiUAT)
          Connection.evmApiUAT = this.createEvmConnection(networkUAT)
