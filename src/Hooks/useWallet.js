...
export default function UseWallet() {
          ...
          getCurrentTabUrl((url) => {
            if (!(url === 'chrome://extensions')) {
              Browser.tabs.sendMessage(id, {
                ...
                response: { evmAddress: address, nativeAddress: nativeAddress },
              })
            }
          })
        })
  ...
  const getBalance = async (evm_api, native_api, isHttp = true) => {
    try {
      ...
      //Native Balance
      if (isHttp) {
        let balance_ = await native_api?._query.system.account(
  ...
  const evmTransfer = async (evmApi, data, isBig = false) => {
    return new Promise(async (resolve, reject) => {
      const tempAmount = isBig
        ? new BigNumber(data.amount).dividedBy(DECIMALS).toString()
        : data.amount
      let dataToDispatch = {
        data: {
          isEvm: true,
          ...
          amount:
            data.amount !== '0x0' ? (isBig ? tempAmount : data.amount) : 0,
          type: data.to
            ? data.amount !== '0x0'
              ? TX_TYPE.SEND
              : 'Contract Execution'
      ...
      try {
        if (
          (Number(tempAmount) > Number(balance.evmBalance) &&
            data.amount !== '0x0') ||
          Number(balance.evmBalance) <= 0
        ) {
          resolve({
            error: true,
          ...
          const transactions = {
            ...
            value: isBig ? data.amount : Number(amt).noExponents().toString(),
            gas: 21000,
            data: data?.data,
          ...
          if (hash) {
            const txRecipt = await httpRequest(
              httpEndPoints[currentNetwork.toLowerCase()],
              HTTP_METHODS.POST,
              JSON.stringify(
                new EVMRPCPayload(EVM_JSON_RPC_METHODS.GET_TX_RECIPT, [hash]),
              ),
            )

            let txStatus = STATUS.PENDING
            if (txRecipt.result) {
              txStatus = Number(txRecipt.result.status)
                ? STATUS.SUCCESS
                : STATUS.PENDING
            }
            ...
            Browser.runtime.sendMessage({
              ...
              ...dataToDispatch,
              statusCheck: {
                isFound: txStatus !== STATUS.PENDING,
          ...
          if (isHttp) {
            transfer.signAndSend(alice, async (txHash) => {
              if (txHash) {
                ...
                dataToDispatch.data.txHash = hash
                const txRecipt = await httpRequest(
                  api[currentNetwork?.toLowerCase()] + hash,
                  HTTP_METHODS.GET,
                )
                ...
                Browser.runtime.sendMessage({
                  ...
                  ...dataToDispatch,
                  statusCheck: {
                    isFound: txStatus !== STATUS.PENDING.toLowerCase(),
                    status: txStatus,
                  },
                })
        ...
        if (
          ...
          if (signHash) {
            ...
            const txRecipt = await httpRequest(
              ...
              HTTP_METHODS.POST,
              JSON.stringify(
                new EVMRPCPayload(EVM_JSON_RPC_METHODS.GET_TX_RECIPT, [
                  signHash,
                ]),
              ),
            )
  ...
  const nominatorValidatorPayout = async (
    nativeApi,
    payload,
    isFee = false,
  ) => {
  ...
  const unbondNominatorValidator = async (
    nativeApi,
    payload,
    isFee = false,
  ) => {
  ...
  const withdrawNominatorValidatorData = async (
    nativeApi,
    payload,
    isFee = false,
  ) => {
  ...
  const withdrawNominatorUnbonded = async (
    nativeApi,
    payload,
    isFee = false,
  ) => {
    try {
      ...
      // }
      // const sendAmountNoExp = Number(amount).noExponents()
  ...
  const addValidator = async (nativeApi, payload, isFee = false) => {
    try {
      ...
      const stashId = encodeAddress(
        decodeAddress(currentAccount?.nativeAddress),
      )
      const commission =
