...
export class Controller {
    ...
    try {
      ...
      if (data.tabId) {
        ...
        Browser.tabs.sendMessage(data.tabId, {
          ...
          response: {
            result:
              storage.auth.httpEndPoints[
                storage.auth.currentNetwork.toLowerCase()
              ],
          },
          error: null,
        })
  ...
  async handleConnect(data) {
    ...
    if (hereOutput.popupStatus) {
      Browser.tabs.sendMessage(data.tabId, {
        ...
        response: null,
        error:
          '5ire extension transaction approve popup session is already active',
      })
      return
    }

    const isEthReq =
      data?.method === 'eth_requestAccounts' || data?.method === 'eth_accounts'
    ...
    if (isExist?.isConnected) {
      const res = isEthReq
        ? {
            method: data?.method,
            result: [state.auth.currentAccount.evmAddress],
          }
        : {
            result: {
  ...
  async handleDisconnect(data) {
    this.store.dispatch(
      toggleSite({ origin: data.message?.origin, isConnected: false }),
    )
    Browser.tabs.sendMessage(data.tabId, {
      id: data.id,
...
function showNotification(controller, message) {
  if (!isNullorUndef(controller) && isHasLength(message))
...
export async function checkTransactions(txData) {
  try {
    ...
    const txHash = isObject(txData.txHash)
      ? txData.txHash.mainHash
      : txData.txHash

    if (txData.statusCheck.isFound) {
      showNotification(
        controller,
        `Transaction ${txData.statusCheck.status} ${txHash.slice(0, 30)} ...`,
      )
      return
    }
    ...
    const rpcUrl = txData.isEVM
      ? state.auth.httpEndPoints[txData.chain] ||
        'https://rpc-testnet.5ire.network'
      : state.auth.api[state.auth.currentNetwork.toLowerCase()]

    //check if the transaction is still pending or not
    let txRecipt
    if (txData.isEVM)
      txRecipt = await httpRequest(
        rpcUrl,
        HTTP_METHODS.POST,
        JSON.stringify(
    ...
    if (txRecipt?.result) {
      store.dispatch(
        updateTxHistory({
          txHash,
          accountName,
          status: Boolean(parseInt(txRecipt.result.status)),
          isSwap,
        }),
      )
      showNotification(
        controller,
        `Transaction ${
          parseInt(txRecipt.result.status)
      ...
      txRecipt?.data &&
      txRecipt?.data?.transaction.status.toLowerCase() !==
        STATUS.PENDING.toLowerCase()
    ) {
      let amount
      if (txRecipt?.data?.transaction?.sectionmethod === 'staking.Rewarded') {
        amount = formatNum(
          Number(noExponents(Number(txRecipt?.data?.transaction?.value))) /
            10 ** 18,
          6,
        )
      }
      ...
      store.dispatch(
        updateTxHistory({
          txHash,
          accountName,
          status: txRecipt?.data?.transaction.status,
          isSwap,
          amount,
        }),
      )
      showNotification(
        controller,
        `Transaction ${txRecipt?.data?.transaction.status} ${txHash.slice(
          0,
          30,
        )} ...`,
      )
...
export async function checkPendingTxns(store) {
  try {
    ...
    for (const acc of allAccounts) {
      const pendingTxn = acc?.txHistory?.filter(
        (tx) => tx?.status?.toLowerCase() === STATUS?.PENDING?.toLowerCase(),
      )
    ...
    for (const txData of pendingList) {
      ...
      const rpcUrl = txData.isEVM
        ? state.auth.httpEndPoints[txData.chain] ||
          'https://rpc-testnet.5ire.network'
        : state.auth.api[txData?.chain.toLowerCase()]
