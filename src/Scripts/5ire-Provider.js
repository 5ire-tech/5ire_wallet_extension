...
export class FireProvider extends SafeEventEmitter {
  constructor(httpHost = 'https://rpc-testnet.5ire.network') {
    ...
    this.conntectMethods = ['eth_requestAccounts', 'eth_accounts', 'connect']
  ...
  sendJsonRpc(method, message = [], isCb = false, cb = null, isFull = false) {
      ...
      try {
        ...
        if (this.restricted.indexOf(method) < 0) {
          const rawResponse = await fetch(
            this.httpHost?.includes('http://') ||
              this.httpHost?.includes('https://')
              ? this.httpHost
