  ...
  handleNativeAppsTask = async (state, message, isFee) => {
    ...
    if (hasProperty(ValidatorNominatorHandler.instance, method)) {
      ...
      if (!isFee) {
        ...
        const network =
          message?.transactionHistoryTrack.chain?.toLowerCase() ||
          state.currentNetwork.toLowerCase();
        const balance = state.allAccountsBalance[message.options?.account.evmAddress][network];
        if (
          Number(methodDetails.amount) >=
          Number(balance?.nativeBalance) -
            (state.pendingTransactionBalance[message.options?.account.evmAddress][network].native -
              Number(methodDetails.amount))
        )
          new Error(
            new ErrorPayload(ERRCODES.INSUFFICENT_BALANCE, {
              error: true,
        ...
        if (Number(methodDetails.amount) < 0 || isNaN(Number(methodDetails.amount)))
          new Error(
            new ErrorPayload(ERRCODES.INVALID_INPUT, {
              error: true,
              data: ERROR_MESSAGES.INVALID_AMOUNT
            })
          ).throw();
      }
      ...
      if (isFee) payload.data = { fee: res.data, ...methodDetails };
        ...
        const transactionHistory = {
          ...message?.transactionHistoryTrack,
