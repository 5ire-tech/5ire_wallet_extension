...
injectedStream.on('data', (data) => {
  ...
  if (data.id) {
    ...
    if (data.error) {
      ...
      if (
        fireProvider.conntectMethods.find((item) => item === handler?.method)
      ) {
        fireProvider.injectSelectedAccount(
          data?.response?.evmAddress ||
            (data?.response?.result?.length && data?.response?.result[0]),
        )
