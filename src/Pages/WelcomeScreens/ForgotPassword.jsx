...
function ForgotPassword() {
  ...
  const [data, setData] = useState({ pass: EMTY_STR, confirmPass: EMTY_STR, key: EMTY_STR, accName: EMTY_STR });
  const [error, setError] = useState({ pass: EMTY_STR, confirmPass: EMTY_STR, key: EMTY_STR, accName: EMTY_STR });
  ...
  const validateAccName = () => {
    ...
    if (data.accName.trim().length < 2 || data.accName.trim().length >= 16) {
      ...
      setError(p => ({ ...p, accName: ERROR_MESSAGES.ALPHANUMERIC_CHARACTERS }))
      // setDisable(true);
    }
    else {
  ...
  const handleSubmit = () => {
    if (!error.key && !error.pass && !error.confirmPass && !error.accName && data.key && data.pass && data.confirmPass && data.accName) {

      sendRuntimeMessage(MESSAGE_TYPE_LABELS.EXTENSION_UI_KEYRING, MESSAGE_EVENT_LABELS.FORGOT_PASS, { password: data.pass, opts: { mnemonic: data.key, name: data.accName.trim() }, type : "forgot"});
