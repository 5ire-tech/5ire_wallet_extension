...
function Send() {
  ...
  const { balance, currentAccount, httpEndPoints, currentNetwork } =
  ...
  useEffect(() => {
    if (gassFee === '' || !gassFee) {
      ...
      if (activeTab.toLowerCase() === EVM.toLowerCase()) {
        if (
          Number(data.amount) + Number(gassFee) >=
  ...
  const validateAmount = () => {
    if (!data.amount) setErr((p) => ({ ...p, amount: INPUT.REQUIRED }))
  ...
  const validateToAddress = async () => {
    if (activeTab.toLowerCase() === EVM.toLowerCase()) {
      if (!data.to) setErr((p) => ({ ...p, to: INPUT.REQUIRED }))
      ...
      .then(async (apiRes) => {
        if (!apiRes?.value) {
          ...
          if (activeTab.toLowerCase() === NATIVE.toLowerCase()) {
            let feeRes = await retriveNativeFee(
              apiRes.nativeApi,
              data.to,
              data.amount,
            )
            ...
            let feeRes = await retriveEvmFee(
              apiRes.evmApi,
              data.to,
              data.amount,
            )
