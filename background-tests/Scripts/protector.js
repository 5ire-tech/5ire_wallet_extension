...
async function encrypt(password, dataObj) {
  ...
  let salt = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : generateSalt();
...
async function encryptWithDetail(password, dataObj) {
  let salt = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : generateSalt();
...
async function decryptWithKey(key, payload) {
  ...
  try {
    ...
    new _error_helper.Error(new _error_helper.ErrorPayload(_index.ERRCODES.INVALID_INPUT, _index.ERROR_MESSAGES.INCORRECT_PASS)).throw();
  }
  return decryptedObj;
}
...
async function keyFromPassword(password, salt) {
  let exportable = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  const passBuffer = Buffer.from(password, STRING_ENCODING);
  const saltBuffer = Buffer.from(salt, "base64");
  const key = await global.crypto.subtle.importKey("raw", passBuffer, {
    name: "PBKDF2"
...
function generateSalt() {
  let byteCount = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 32;
