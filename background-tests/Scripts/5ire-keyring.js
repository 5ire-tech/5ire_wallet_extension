...
var protector = _interopRequireWildcard(require("./protector"));
var _common = require("@ethereumjs/common");
var _ethHdKeyring = _interopRequireDefault(require("@metamask/eth-hd-keyring"));
var utilCrypto = _interopRequireWildcard(require("@polkadot/util-crypto"));
...
class HybridKeyring extends _events.EventEmitter {
  static initKeyring() {
    HybridKeyring.polkaKeyring = new _keyring.default();
    HybridKeyring.ethKeyring = new _ethHdKeyring.default();
    HybridKeyring.simpleEthKeyring = new _ethSimpleKeyring.default();
  }
  ...
  async loadPersistData(password, vault) {
    ...
    if (res.vault && res.vault.length > 0) {
      ...
      for (const v of res.vault) {
        if (v.type === _Constants.WALLET_TYPES.HD) {
        ...
        } else if (v.type === _Constants.WALLET_TYPES.ETH_SIMPLE) {
        ...
        } else if (v.type === _Constants.WALLET_TYPES.IMPORTED_NATIVE) {
          const oldKeys = await HybridKeyring.simpleEthKeyring.serialize();
  ...
  async verifyUserPassword(message) {
    var _message$data;
    const verifiedResponse = await this._verifyPassword(message === null || message === void 0 ? void 0 : (_message$data = message.data) === null || _message$data === void 0 ? void 0 : _message$data.password);
    const verified = verifiedResponse.vault ? true : false;
    return new _network_calls.EventPayload(message.event, message.event, {
      verified
    });
  }
  ...
  async recoverOldStateAccounts(message) {
    ...
    let keyResponse = {};
    if (!password) {
      if (HybridKeyring !== null && HybridKeyring !== void 0 && HybridKeyring.password) password = HybridKeyring.password;else throw new _error_helper.Error(_Constants.ERROR_MESSAGES.PASS_REQUIRED);
    }
    ...
    return new _network_calls.EventPayload(message.event, message.event, payload, false);
  }
  /**
  ...
  async createOrRestore(message) {
    ...
    if (!password) {
      if (HybridKeyring.password) password = HybridKeyring.password;else throw new _error_helper.Error(_Constants.ERROR_MESSAGES.PASS_REQUIRED);
    }
    ...
    HybridKeyring.password = password;
    this._initKeys(opts);
    const data = this._getKeyringData(_Constants.WALLET_TYPES.HD);
    ...
    for (let i = 0; i < data.numberOfAccounts; i++) {
      const acc = {
        ...
        evmAddress: _web.default.utils.toChecksumAddress(ethAccounts[i]),
        type: _Constants.WALLET_TYPES.HD,
        accountName: (opts === null || opts === void 0 ? void 0 : opts.name) || _Constants.WALLET_TYPES.HD + "_" + i,
        accountIndex: i
      };
    ...
    return new _network_calls.EventPayload(message.event, message.event, payload);
  }
  ...
  async addAccount(message) {
    ...
    if (HybridKeyring.accounts.length <= 0) {
      throw new _error_helper.Error(_Constants.ERROR_MESSAGES.NO_ROOT_ACC);
    }
    const keyring = this._getKeyringData(_Constants.WALLET_TYPES.HD);
    const eth = await HybridKeyring.ethKeyring.addAccounts(1);
    //Check existing accounts in HD category
    const oldAccounts = HybridKeyring.accounts.filter(acc => acc.type === _Constants.WALLET_TYPES.HD);
    const existingHdAccounts = keyring.numberOfAccounts;
    const mainPair = HybridKeyring.polkaKeyring.getPair((_oldAccounts$ = oldAccounts[0]) === null || _oldAccounts$ === void 0 ? void 0 : _oldAccounts$.nativeAddress);
    const newKr = mainPair.derive("//" + existingHdAccounts);
    HybridKeyring.polkaKeyring.addPair(newKr);
    const newAcc = {
      nativeAddress: newKr.address,
      evmAddress: _web.default.utils.toChecksumAddress(eth[0]),
      type: _Constants.WALLET_TYPES.HD,
      accountName: name || _Constants.WALLET_TYPES.HD + "_" + existingHdAccounts,
    ...
    this.emit(_Constants.KEYRING_EVENTS.ACCOUNT_ADDED, newAcc);
    const keyResponse = await this._exportEthAccountByAddress(newAcc.evmAddress, HybridKeyring.password);
    newAcc.evmPrivateKey = keyResponse ? keyResponse : "";
   ...
   * @returns
   */
  async importAccountByPrivateKey(pvtKey) {
    ...
    if (isExist) throw new _error_helper.Error(_Constants.ERROR_MESSAGES.ACCOUNT_EXISTS);

    //Handle Keyring
    let keyring = this._getKeyringData(_Constants.WALLET_TYPES.ETH_SIMPLE);
    if (keyring) {
      ...
      const newKeyring = {
        numberOfAccounts: 1,
        private_keys: [pvtKey],
        type: _Constants.WALLET_TYPES.ETH_SIMPLE,
        accounts: []
      };
    ...
    const ethAddress = _web.default.utils.toChecksumAddress(accounts[newAccountIndex]);
    ...
    const newAcc = {
      ...
      type: _Constants.WALLET_TYPES.ETH_SIMPLE,
      accountname: name || _Constants.WALLET_TYPES.ETH_SIMPLE + "_" + keyring.numberOfAccounts,
  ...
  async importAccountByMnemonics(message) {
    ...
    if (isExist) {
      ...
      return new _error_helper.Error(new _error_helper.ErrorPayload(_Constants.ERRCODES.INVALID_INPUT, _Constants.ERROR_MESSAGES.MNEMONICS_ALREADY_EXISTS)).throw();
    }

    //Handle Keyring
    let keyring = this._getKeyringData(_Constants.WALLET_TYPES.IMPORTED_NATIVE);
    if (keyring) {
      ...
      const newKeyring = {
        numberOfAccounts: 1,
        mnemonics: [mnemonic],
        type: _Constants.WALLET_TYPES.IMPORTED_NATIVE,
        accounts: []
      };
      HybridKeyring.keyrings.push(newKeyring);
      keyring = newKeyring;
    ...
    oldKeys.push(keyWallet.privateKey);
    await HybridKeyring.simpleEthKeyring.deserialize(oldKeys);
    const ethAddress = _web.default.utils.toChecksumAddress(keyWallet.address);
    ...
    const newAcc = {
      ...
      evmAddress: ethAddress,
      type: _Constants.WALLET_TYPES.IMPORTED_NATIVE,
      accountName: name || _Constants.WALLET_TYPES.IMPORTED_NATIVE + "_" + keyring.numberOfAccounts,
      accountIndex: keyring.accounts.length
    };
    HybridKeyring.accounts.push(newAcc);
  ...
  async removeAccount(message) {
    var _message$data2, _message$data3, _message$data4;
    const address = message === null || message === void 0 ? void 0 : (_message$data2 = message.data) === null || _message$data2 === void 0 ? void 0 : _message$data2.address;
    const password = message !== null && message !== void 0 && (_message$data3 = message.data) !== null && _message$data3 !== void 0 && _message$data3.password ? message === null || message === void 0 ? void 0 : (_message$data4 = message.data) === null || _message$data4 === void 0 ? void 0 : _message$data4.password : HybridKeyring.password;
    await this._verifyPassword(password);
    const info = HybridKeyring.accounts.find(acc => acc.evmAddress === address || acc.nativeAddress === address);
    if (!info) throw new _error_helper.Error(_Constants.ERROR_MESSAGES.NO_ACC_EXISTS_WITH_THIS_ADDR);
    const keyring = this._getKeyringData(info.type);
    if (info.type === _Constants.WALLET_TYPES.HD) {
      // HybridKeyring.ethKeyring.removeAccount(info.evmAddress)
    } else if (info.type === _Constants.WALLET_TYPES.ETH_SIMPLE) {
      HybridKeyring.simpleEthKeyring.removeAccount(info.evmAddress);
      keyring.private_keys.splice(info.accountIndex, 1);
      keyring.numberOfAccounts--;
    } else if (info.type === _Constants.WALLET_TYPES.IMPORTED_NATIVE) {
      HybridKeyring.simpleEthKeyring.removeAccount(info.evmAddress);
  ...
  async forgotPassByMnemonic(message) {
    try {
      ...
      return new _error_helper.Error(new _error_helper.ErrorPayload(_Constants.ERRCODES.INTERNAL, _Constants.ERROR_MESSAGES.INVALID_PROPERTY)).throw();
    }
  }
  ...
  async removeNativeAccount(password, address) {
    ...
    if (!info) throw new _error_helper.Error(_Constants.ERROR_MESSAGES.NO_ACC_EXISTS_WITH_THIS_ADDR);
    const keyring = this._getKeyringData(info.type);
    if (info.type === _Constants.WALLET_TYPES.ETH_SIMPLE) {
      keyring.private_keys.splice(info.accountIndex, 1);
    } else if (info.type === _Constants.WALLET_TYPES.IMPORTED_NATIVE) {
      keyring.mnemonics.splice(info.accountIndex, 1);
    }
   ...
   */
  getAccounts(message) {
    return new _network_calls.EventPayload(null, message.event, HybridKeyring.accounts);
  }
  ...
  async signEthTx(address, tx) {
    ...
    const txn = _tx.TransactionFactory.fromTxData(tx, {
      common
    });
    let signedTx = null;
    if (acc.type === _Constants.WALLET_TYPES.HD) {
      signedTx = await HybridKeyring.ethKeyring.signTransaction(address, txn);
    } else if (acc.type === _Constants.WALLET_TYPES.ETH_SIMPLE || acc.type === _Constants.WALLET_TYPES.IMPORTED_NATIVE) {
      signedTx = await HybridKeyring.simpleEthKeyring.signTransaction(address, txn);
    } else {
      throw new _error_helper.Error(_Constants.ERROR_MESSAGES.INCORRECT_ADDRESS);
    }
  ...
  async _exportNativeAccountByAddress(address, password) {
    ...
    } else if (acc.type === _Constants.WALLET_TYPES.ETH_SIMPLE) {
      const data = this._getKeyringData(_Constants.WALLET_TYPES.ETH_SIMPLE);
      return data.private_keys[acc.accountIndex];
    } else if (acc.type === _Constants.WALLET_TYPES.IMPORTED_NATIVE) {
      const data = this._getKeyringData(_Constants.WALLET_TYPES.IMPORTED_NATIVE);
      return data.mnemonics[acc.accountIndex];
    } else {
      new _error_helper.Error(new _error_helper.ErrorPayload(_Constants.ERRCODES.KEYRING_ERROR, _Constants.ERROR_MESSAGES.INCORRECT_KEYRING)).throw();
    }
  }
  ...
  async _persistData(password) {
    ...
    this.emit(_Constants.KEYRING_EVENTS.STATE_CHANGED, res);
   ...
   */
  _getKeyringData() {
    let type = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _Constants.WALLET_TYPES.HD;
  ...
  _initKeys(opts) {
    HybridKeyring.keyrings.push({
      numberOfAccounts: (opts === null || opts === void 0 ? void 0 : opts.numberOfAccounts) || 1,
      mnemonic: (opts === null || opts === void 0 ? void 0 : opts.mnemonic) || this.generateRandomMnemonics(),
      type: _Constants.WALLET_TYPES.HD,
      accounts: []
    });
  }
}
exports.HybridKeyring = HybridKeyring;
_defineProperty(HybridKeyring, "ethKeyring", void 0);
_defineProperty(HybridKeyring, "vault", null);
_defineProperty(HybridKeyring, "polkaKeyring", void 0);
_defineProperty(HybridKeyring, "password", null);
_defineProperty(HybridKeyring, "keyrings", []);
_defineProperty(HybridKeyring, "accounts", []);
_defineProperty(HybridKeyring, "instance", null);
_defineProperty(HybridKeyring, "isLocked", false);
_defineProperty(HybridKeyring, "simpleEthKeyring", void 0);
//return the already initlized instance if there is no instance then it will create it.
_defineProperty(HybridKeyring, "getInstance", () => {
